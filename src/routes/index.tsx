    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-6 py-24 text-center">
        ...
        <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
          Compose, schedule, and track every email. Manage SMTP accounts, templates, and recipient
          lists from one clean dashboard.
        </p>
      ...
      <section className="container mx-auto px-6 pb-24 grid md:grid-cols-3 gap-6">
        {[
          {
            icon: Send,
            title: "Single & Bulk Send",
            desc: "Compose one-off emails or upload CSV/XLSX for thousands at once.",
          },
          {
            icon: BarChart3,
            title: "Real-time Analytics",
            desc: "Track sent, pending, failed and per-campaign performance.",
          },
          {
            icon: ShieldCheck,
            title: "Secure SMTP",
            desc: "Encrypted SMTP credentials and per-org isolation built in.",
          },
        ].map((f) => (
          <div key={f.title} className="rounded-2xl border border-border bg-card p-6">
            <div
              className="h-10 w-10 rounded-lg grid place-items-center text-primary-foreground mb-4"
              style={{ background: "var(--gradient-primary)" }}
            >
